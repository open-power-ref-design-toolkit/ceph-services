...
import copy
from ulysses_ceph import generate_ceph_ansible_input as test_mod
import os.path
import mock
import unittest
...
class TestGenerateCephAnsibleInput(unittest.TestCase):
    TEST_MODULE_STRING = 'ulysses_ceph.generate_ceph_ansible_input'
    ...
    def test_get_openstack_pools(self, osd_count, pg_count):
        ...
        pg_count.assert_has_calls([mock.call(50, .25, 100),
                                   mock.call(50, .60, 100),
                                   mock.call(50, .15, 100)])
    ...
    def test_generate_files(self, load_yml, all_vars, osds, hosts,
        ...
        write_yml.assert_has_calls(
            [mock.call(root_dir+os.path.sep+'group_vars'+os.path.sep+'all',
                       all_vars.return_value),
             mock.call(root_dir+os.path.sep+'group_vars'+os.path.sep+'osds',
                       osds.return_value)])
        write_string.assert_called_once_with(root_dir+os.path.sep+'ceph-hosts',
                                             hosts.return_value)
